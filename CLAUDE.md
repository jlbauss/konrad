...
- **`.agent/` belongs to the agent end-to-end.** Layout: `.agent/task.md` (planning artifact, committable) · `.agent/artifacts/` (durable mid-task outputs like `manual-output.<ext>`, committable) · `.agent/scratch/` (agent-written scripts/probes, auto-pruned >7d, gitignored) · `.agent/qa/<stamp>/` (QA evidence, auto-pruned >7d, gitignored). No more `.agent/opencode/` — opencode session state is now ephemeral inside the container; logs land in `~/.local/state/konrad/log/` on the host. See [docs/design/state-isolation.md](docs/design/state-isolation.md) for the contract.
