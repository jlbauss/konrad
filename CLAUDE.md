...
- **Validate smoke locally before pushing CI changes.** Editing `scripts/smoke-test.sh`, `image/Dockerfile`, or `image/build-manifest.sh`? Run `konrad rebuild && ./scripts/smoke-test.sh konrad:local` before pushing. CI catches the bug at ~10 min round-trip; local catches it at ~5.5 min. Cross-reference Debian package names vs binary names when listing things to check (`ripgrep`→`rg`, `fd-find`→`fdfind`, `bat`→`batcat`).
