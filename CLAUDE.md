...
- **Fetch before you start editing, and rebase before every push handoff — `main` grows commits on its own.** The resolve-locks bot auto-pushes `chore(locks)` commits to `main` on its own schedule, so your base goes stale even mid-session. `git fetch origin` before beginning a set of edits; and again before handing over a push — if `main` is behind, `git rebase origin/main` so your work replays on the current tip and the push is a clean fast-forward (re-run the affected gates after the rebase). Never hand the user a push that isn't fast-forwardable.
...
- **Default to validating in the dev container before handing the user a host probe.** The host podman socket + real images mean you can exercise most runtime and CLI behavior here yourself — reserve a host-side probe for the genuinely host-only (a *fresh-machine* install + `konrad update` round-trip, the release-testing item's L4). The **update/refresh path in particular is testable with no host and without touching the real `:latest`:** `KONRAD_REGISTRY_IMAGE` overrides the pull *source* for `require_image` / `do_pull_image` / `do_update` / the background refresh (distinct from `KONRAD_IMAGE`, which sets what *runs*), so point it at a `:pr-<num>` candidate or a local `registry:2` of tiny dummy images. Auto-refresh is gated off `konrad-dev` (dev-ness = `argv[0]`), so drive it either by running the script under its stable name (a copy named `konrad`) with an isolated `HOME`, or by sourcing the functions extracted from `bin/konrad` against real podman — both hit the real code path. The reusable rehearsal is tracked as `scripts/test-autoupdate.sh` in the Release-testing procedure ([ROADMAP.md](ROADMAP.md)).
- **Run any `bin/konrad` test harness under the same shell options the script uses — `set -euo pipefail`.** A harness without them silently *passes* code that aborts in production: the 0.22.0 first-launch crash was a `head <missing-file> | …` read that's fatal only under `set -e`+`pipefail` (a missing file makes the pipeline non-zero → `set -e` kills the launch), and the harness that "validated" it ran without those options, so it never reproduced the abort. Set them at the top of the harness, and cover the empty/missing/malformed-input cases explicitly.
