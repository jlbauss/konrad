...
- **Trunk-based, PR-gated development.** Branch off `main` (`feat/<short>`, `fix/<short>`, `docs/<short>`, `chore/<short>`), commit, push, open a PR on GitHub. CI builds the PR's image as `ghcr.io/jlbauss/konrad:pr-<num>` (same-repo PRs only — fork PRs run smoke but don't publish). Test interactively with `KONRAD_IMAGE=ghcr.io/jlbauss/konrad:pr-<num> konrad`. Merge when smoke is green and the PR image works; main pipeline then publishes under the real tags. Direct pushes to `main` are allowed but only for same-day iteration on something already in-flight. See [CONTRIBUTING.md](CONTRIBUTING.md) for the full workflow.
